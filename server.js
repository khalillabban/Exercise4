...
app.post("/createAnAccount", (req, res) => {
    ...
    if (!username || !password) {
        return res.render("createAnAccount", { message: "Username and password are required" });
    }
    ...
    fs.readFile(filePath, "utf8", (err, data) => {
        ...
        if (data.includes(`${username}:`)) {
            return res.render("createAnAccount", { message: "Username already exists" });
        }
        ...
        fs.appendFile(filePath, userString, (err) => {
            ...
            res.render("createAnAccount", { message: "Account created successfully" });
        });
    });
});
...
app.post("/findpets", (req, res) => {
    ...
    fs.readFile(filePath, "utf8", (err, data) => {
        ...
        const filteredPets = petsData.filter(line => {
            ...
            return (!species || parts[0].toLowerCase() === species.toLowerCase()) &&
                (!breed || breed === "No preference" || parts[1].toLowerCase().includes(breed.toLowerCase())) &&
...
app.post("/registerPet", (req, res) => {
    const { species, breed, ageCategory, gender, social, more, name, email } = req.body;
